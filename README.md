...
Garmin remains the primary source for activity weather. An account can opt in under Settings to use Open-Meteo when Garmin returns no usable conditions. The fallback sends rounded start coordinates and the activity date to Open-Meteo, stores the nearest hourly model result, and attributes it in the activity detail and Copy for AI output. The built-in limiter stays below the [free non-commercial API limits](https://open-meteo.com/en/terms); commercial deployments need an appropriate Open-Meteo plan. Open-Meteo data is provided under [CC BY 4.0](https://open-meteo.com/en/license).
