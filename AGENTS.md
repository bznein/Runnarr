...
- For a material user-facing change, a write-level collaborator may apply one or two matching `visual:<viewport>:<scenario>` PR labels from `.github/visual-review-profiles.json`. New screens, navigation or interaction changes, responsive behavior, multi-component layout work, maps/charts, and changed loading, empty, or error states normally qualify. Backend-only work, docs/tests, invisible refactors, copy-only corrections, and genuinely tiny isolated styling fixes normally do not. If uncertain, prefer recording the affected journey.
- Treat visual labels as approval for one exact PR head. A push, reopen, or ready-for-review event performed by an actor without write access removes the labels; a write-level collaborator must review the new head and reapply the affected profiles. Do not bypass that reapproval by restoring labels with an untrusted account.
...
- When visual review is requested, do not hand off the PR until the current-head bot comment embeds the bounded before/after recordings (or its diagnostic ZIP fallback) or an unavoidable workflow blocker has been reported.
