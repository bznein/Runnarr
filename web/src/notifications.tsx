...
import { Bell, ChevronDown, ChevronUp, ExternalLink, Send, Trash2 } from "lucide-react";
import { useInfiniteQuery, useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useLocation, useNavigate } from "react-router-dom";
import { api } from "./api";
import type { NotificationCategory, NotificationMode, PushSubscriptionDevice, RunnarrNotification } from "./types";
...
export function NotificationBell({ mobile = false, canWrite = true }: { mobile?: boolean; canWrite?: boolean }) {
  ...
  const markAll = useMutation({ mutationFn: api.markAllNotificationsRead, onSuccess: () => invalidateNotifications(queryClient) });
    ...
    <div className={`notification-bell ${mobile ? "mobile-notification-bell" : ""}`}>
      ...
      {open && (
        <div className="notification-popover" role="dialog" aria-label="Recent notifications">
          <div className="notification-popover-heading"><strong>Notifications</strong>{canWrite && unreadCount > 0 && <button className="text-button" type="button" disabled={markAll.isPending} onClick={() => markAll.mutate()}>Mark all read</button>}</div>
          <div className="notification-popover-list">
            {(notifications.data?.notifications ?? []).map((item) => <button key={item.id} className={`notification-popover-item ${item.readAt ? "" : "unread"}`} type="button" onClick={() => void openNotification(item)}><span className={`notification-dot ${item.severity}`} /><span><strong>{item.title}</strong>{item.body && <small>{item.body}</small>}<time>{notificationDate(item.lastEventAt)}</time></span></button>)}
            {notifications.error && <div className="notification-empty error-text">Could not load notifications.</div>}
            {!notifications.error && !notifications.isLoading && (notifications.data?.notifications.length ?? 0) === 0 && <div className="notification-empty">Nothing to review.</div>}
          </div>
...
export function NotificationsPage({ canWrite = true }: { canWrite?: boolean }) {
  ...
  const clear = useMutation({ mutationFn: api.clearNotifications, onSuccess: refresh });
  ...
  return (
    <div className="page notifications-page">
      ...
      <div className="notification-toolbar">
        ...
        {canWrite && <div className="notification-toolbar-actions"><button className="secondary-button small-button" type="button" onClick={() => markAll.mutate()}>Mark all read</button><button className="secondary-button small-button" type="button" onClick={() => clearScope("read")}>Clear read</button><button className="danger-button small-button" type="button" onClick={() => clearScope("all")}>Clear all</button></div>}
      </div>
      <section className="notification-list">
        {items.map((item) => <article key={item.id} className={`panel notification-card ${item.readAt ? "" : "unread"}`}>
          <button className="notification-card-main" type="button" onClick={() => setExpanded((value) => value === item.id ? undefined : item.id)}><span className={`notification-dot ${item.severity}`} /><span className="notification-card-copy"><strong>{item.title}</strong>{item.body && <span>{item.body}</span>}<time>{notificationDate(item.lastEventAt)} · {item.eventCount} {item.eventCount === 1 ? "event" : "events"}</time></span>{expanded === item.id ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</button>
          {expanded === item.id && <div className="notification-timeline">
            {(detail.data?.events ?? []).map((event) => <div key={event.id} className="notification-timeline-event"><span className={`notification-dot ${event.severity}`} /><div><strong>{event.title}</strong>{event.body && <p>{event.body}</p>}<time>{notificationDate(event.createdAt)}</time></div></div>)}
